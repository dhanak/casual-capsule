...
env_flag_enabled() {
  ...
  case "$value" in
    ...
    *) die "$name must be one of: 1, 0, true, false, yes, no, on, off" ;;
  esac
}
...
# Apply environment options shared by run and build commands.
apply_build_option_env() {
  if env_flag_enabled CAPSULE_BUILD; then
    set_build_mode "all" "CAPSULE_BUILD"
  fi
  if env_flag_enabled CAPSULE_BUILD_CUSTOM; then
    set_build_mode "custom" "CAPSULE_BUILD_CUSTOM"
  fi
  if env_flag_enabled CAPSULE_NO_CACHE; then
    NO_CACHE=1
  fi
}

# Apply environment options used only when starting a Capsule.
apply_run_option_env() {
  if env_flag_enabled CAPSULE_PRIVATE_HOME; then
    PRIVATE_HOME=1
  fi
  if env_flag_enabled CAPSULE_HOST_DOCKER; then
    HOST_DOCKER=1
  fi
}
...
usage() {
  cat <<'EOF'
Usage: capsule run [options] [--] [command...]
...
Environment:
  ...
  CAPSULE_BUILD    Enable --build when set to a true value.
...
set_build_mode() {
  ...
  if [[ "$BUILD_MODE_FLAG" == CAPSULE_* ]] && [[ "$new_flag" == -* ]]; then
    BUILD_MODE="$new_mode"
    BUILD_MODE_FLAG="$new_flag"
    return
  fi
...
# Apply the remote target supplied through the environment unless CLI won.
apply_remote_env() {
  if [[ -z "$REMOTE_HOST" ]] && [[ -n "${CAPSULE_REMOTE:-}" ]]; then
    parse_remote_target "$CAPSULE_REMOTE"
  fi
}

# Require a remote workdir for run and build operations.
validate_operation() {
  local remote_err='--remote requires HOST[:PORT]:/absolute/workdir'

  if [[ "$BUILD_ONLY" -ne 1 ]] && \
    [[ -n "$REMOTE_HOST" ]] && [[ -z "$REMOTE_WORKDIR" ]]; then
    die "$remote_err"
  fi
}
...
main() {
  ...
  if [[ "$BUILD_ONLY" -eq 1 ]] && [[ "$BUILD_MODE" == "none" ]]; then
    set_build_mode "all" "build command"
  fi
  validate_operation
